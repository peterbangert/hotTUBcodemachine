...
def adjusted_filename(name):
    suffix = raw_input("Add suffix to copied file? : ")
...
def edit():
    yn = raw_input("Standard googletranslate file edit ? (y/n) : ")
    if (yn.lower() == "y"):
        find_replace()
        return

    yn = raw_input(".text to .md reformat ? (y/n) : ")
    if (yn.lower() == "y"):
        txt_to_md()
        return    
...
def count_words(path, words, visited):
    mustContain = raw_input("What must filename conatin? : ")
    for root, dirs, files in os.walk(path):
...
def search_string(path):
    ...
    args = raw_input("what search for plz? : ").split(",")
    print args
    for root, dirs, files in os.walk(path):
        for name in files:
            searchable = args
...
def move():
    filename = raw_input("filename plz, with extension : ")
    dest = raw_input("destination plz, just folder name : ")
...
def replace():
    while (True):
        searchText = raw_input("Text to Replace : ")
        replaceText = raw_input("Replace with : ")
